...
// ==============================================================================
// HELPER: RICH HTML PRODUCT CALLOUT CARD GENERATOR
// ==============================================================================
...
<div class="product-card-box my-8 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-900 dark:via-slate-950 dark:to-slate-900 shadow-xl transition-all duration-300 hover:shadow-2xl hover:border-cyan-500/50" style="margin: 2rem 0; padding: 1.5rem; border-radius: 1rem; border: 1px solid rgba(226, 232, 240, 0.8); background: #ffffff; box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.05), 0 8px 10px -6px rgba(0, 0, 0, 0.01);">
  <div style="display: flex; flex-direction: row; flex-wrap: wrap; gap: 1.5rem; align-items: center;">
    ...
    <div style="flex: 1; min-width: 260px;">
      ...
      <h4 style="margin: 0 0 0.5rem 0; font-size: 1.15rem; font-weight: 800; color: #0f172a; line-height: 1.3;">${title}</h4>
      <p style="margin: 0 0 1rem 0; font-size: 0.875rem; color: #475569; line-height: 1.5;">${subtitle}</p>
      <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1rem; border-top: 1px solid #f1f5f9; padding-top: 0.75rem;">
        <div>
          ...
          <span style="font-size: 1.25rem; font-weight: 800; color: #0f172a;">${price} <span style="font-size: 0.8rem; font-weight: 600; color: #64748b;">${priceSecondary ? `(${priceSecondary})` : ''}</span></span>
        </div>
        <a href="${storeUrl}" target="_blank" rel="noopener noreferrer" style="display: inline-flex; align-items: center; gap: 0.5rem; background: linear-gradient(135deg, #06b6d4 0%, #0891b2 100%); color: #ffffff; padding: 0.6rem 1.25rem; border-radius: 0.5rem; font-weight: 700; font-size: 0.875rem; text-decoration: none; box-shadow: 0 4px 12px rgba(6, 182, 212, 0.3); transition: all 0.2s ease;">
...
// Audit Word Counts
console.log('==================================================================');
console.log(' V3 MASTERCLASS CLINICAL ARTICLES WORD COUNT & CARD AUDIT');
console.log('==================================================================');
for (const a of articles) {
  const wordCount = a.content.trim().split(/\s+/).length;
  const cardCount = (a.content.match(/product-card-box/g) || []).length;
  console.log(`Pillar [${a.slug}]: ~${wordCount} words | ${cardCount} Product Cards ✅`);
}
