...
async function main() {
  ...
  for (const cat of categories) {
    ...
    if (postError) {
      console.error(`Error fetching post for ${cat}:`, postError.message);
      continue;
    }
  ...
  for (const subscriber of subscribers) {
    ...
    if (selectedPosts.length === 0) {
      console.log(`No matching posts for subscriber ${subscriber.email} based on preferences. Skipping.`);
      continue;
    }
    ...
    const emailBody = `
      ...
      <head>
        <meta charset="utf-8">
        <title>Weekly Bento Box Digest</title>
      </head>
                ...
                <tr>
                  <td bgcolor="#f1f5f9" style="padding: 24px 24px; text-align: center; border-radius: 0 0 16px 16px; border-top: 1px solid #e2e8f0;">
                    <p style="margin: 0 0 8px 0; font-size: 11px; color: #64748b; line-height: 1.5;">
                      Sent to ${subscriber.email} based on your selected interests.
                    </p>
                    <p style="margin: 0; font-size: 11px; color: #94a3b8;">
                      To change your topics or unsubscribe, reply "Unsubscribe" or manage preferences in your dashboard.
                    </p>
                  </td>
                </tr>
              </table>
    ...
    try {
      console.log(`Sending bento box newsletter to ${subscriber.email}...`);
      await transporter.sendMail({
        from: smtpFrom,
        to: subscriber.email,
        subject: `123TheNextLevel — Your Weekly Bento Box Digest`,
        html: emailBody
      });
  ...
  console.log('Weekly Bento Box distribution complete.');
}
