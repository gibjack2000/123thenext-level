...
export default function MarketSelector({ currentCategory, className = "" }: MarketSelectorProps) {
  ...
  return (
    <div className={`flex items-center gap-2 sm:gap-4 ${className}`}>
      <span className="hidden sm:inline text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mr-2">Market:</span>
        ...
        {REGIONS.map((r) => {
          ...
          return (
            <Link
              key={r.id}
              to={targetPath}
              className={`group relative flex items-center justify-center w-10 h-8 sm:w-12 sm:h-10 rounded-lg sm:rounded-xl transition-all duration-500 overflow-hidden ${
                isActive 
            ...
            >
              <img 
                src={r.flag} 
                alt={r.name} 
                className={`w-6 sm:w-7 h-auto rounded-sm transition-all duration-500 ${isActive ? 'scale-110 brightness-110' : 'grayscale group-hover:grayscale-0 group-hover:scale-110'}`}
              />
              {isActive && (
