...
import { X, ChevronDown, ChevronUp, AlertCircle, Eye, EyeOff } from 'lucide-react';
...
export default function UrinalysisInstructions({ onDismiss }: UrinalysisInstructionsProps) {
  ...
  if (!isVisible) {
    return (
      <div className="flex justify-end mb-4">
        <button
          onClick={handleReset}
          className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-slate-900 hover:bg-slate-850 border border-slate-800 hover:border-wellness-cyan/30 text-slate-400 hover:text-wellness-cyan-light text-[10px] font-mono uppercase tracking-wider transition-all duration-200 cursor-pointer active:scale-95 shadow-lg shadow-slate-950/40"
        >
          <Eye size={12} className="text-wellness-cyan" />
          <span>Show Diagnostic Manual</span>
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="w-full rounded-3xl bg-slate-900 border border-amber-500/20 p-6 text-left relative overflow-hidden transition-all duration-300 shadow-xl">
      {/* Subtle warning state radial glow background */}
      ...
      <div className="flex justify-between items-start gap-4 border-b border-slate-800/80 pb-4 mb-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-1.5 rounded bg-slate-950 border border-slate-800 hover:border-slate-700 text-slate-400 hover:text-slate-200 transition-all cursor-pointer"
          ...
          <button
            ...
            className="p-1.5 rounded bg-slate-950 border border-slate-800 hover:border-rose-500/30 text-slate-400 hover:text-rose-450 transition-all cursor-pointer"
      ...
      {!isCollapsed && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="p-4 bg-slate-950/40 border border-slate-850 rounded-2xl flex flex-col justify-between">
              <div>
                ...
                <p className="text-[11px] text-slate-grey-300 font-light leading-relaxed">
                  Wash hands thoroughly. Extract one test strip from the vial and <strong className="text-white font-medium">tightly seal the bottle lid immediately</strong> to prevent ambient humidity from damaging the remaining reagent panels.
                </p>
              </div>
            </div>
            ...
            <div className="p-4 bg-slate-950/40 border border-slate-850 rounded-2xl flex flex-col justify-between">
              <div>
                ...
                <p className="text-[11px] text-slate-grey-300 font-light leading-relaxed">
                  Submerge all 10 reagent panels completely in your fresh urine sample for <strong className="text-cyan-400 font-bold">exactly one second</strong>. Ensure all chemical blocks are fully wetted.
                </p>
              </div>
            </div>
            ...
            <div className="p-4 bg-slate-950/40 border border-slate-850 rounded-2xl flex flex-col justify-between">
              <div>
                ...
                <p className="text-[11px] text-slate-grey-300 font-light leading-relaxed">
                  Tap the edge of the strip on a paper towel to drain excess urine. Hold the strip <strong className="text-white font-medium">strictly horizontally</strong> to prevent cross-run, and set a timer for <strong className="text-cyan-400 font-bold">exactly 60 seconds</strong> (<strong className="text-cyan-400 font-bold">120 seconds</strong> for Leukocytes).
                </p>
              </div>
            </div>
            ...
            <div className="p-4 bg-slate-950/40 border border-slate-850 rounded-2xl flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-center mb-2.5">
                  ...
                  <span className="text-[9px] font-mono text-slate-500 uppercase">Logging</span>
                </div>
                <h5 className="text-xs font-bold text-white uppercase tracking-wide mb-1.5">4. Match & Log</h5>
                <p className="text-[11px] text-slate-grey-300 font-light leading-relaxed">
                  Compare the reagent pads to the color blocks printed on the bottle bottle chart. Enter the matching values into the interactive logger below and sync them with your dashboard.
                </p>
              </div>
            </div>
          ...
          <div className="p-4 border border-slate-800 bg-slate-950/40 rounded-2xl flex gap-3 items-start">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[10.5px] sm:text-[11px] text-slate-grey-455 font-light leading-relaxed">
      ...
      {/* Mini preview notification when collapsed */}
      {isCollapsed && (
        <div className="flex items-center justify-between text-slate-400 text-[10px] sm:text-xs font-light">
          <div className="flex items-center gap-2">
            <AlertCircle size={12} className="text-wellness-cyan" />
            <span>Protocol guidelines collapsed. Click the expand button to review step-by-step instructions.</span>
          </div>
          <button 
            onClick={toggleCollapse} 
            className="text-wellness-cyan hover:text-wellness-cyan-light underline font-bold uppercase tracking-wider cursor-pointer text-[10px] font-mono"
          >
            Open Protocol Guide
          </button>
        </div>
